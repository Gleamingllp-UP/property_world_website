           ...
           {agentOrAgencyData.map((agent, index) => (
            <div className="col-lg-5 agency_list" key={index}>
              <div className="agent_info_image agent_info_image2 agent_pennel_2">
                <div>
                  <Link to={pageRoutes.AGENT_INFO}><img src={agent?.agent_photo}className="img-fluid" /></Link>
                </div>
                <div>
                  <h3>{agent?.first_name}{' '}{agent?.last_name}</h3>
                  <p><b>Agent:</b> {agent?.company_name}</p>
                  <p><b>Exp.:</b> {agent?.experience} years <span className="ratt"><i className="ri-star-fill" /> 4.2</span></p>
                  <p><b>Nationally:</b> {agent?.country_of_residance}</p>
                  {agent?.languages?.map((lang, index) => (
                    <p key={index}><b>Language: {lang}</b></p>
